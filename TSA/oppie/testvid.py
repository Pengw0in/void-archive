...
def play_with_mpv(video_url):
    """
    Play a YouTube video directly using MPV without downloading it first
    """
    # Check if MPV is installed
    if shutil.which("mpv") is None:
        print("Error: MPV is not installed. Please install it first.")
        print("On macOS, run: brew install mpv")
        return False
    
    try:
        print(f"Playing {video_url} with MPV...")
        # Use subprocess to call mpv with the YouTube URL
        subprocess.run(["mpv", video_url], check=True)
        return True
    except subprocess.SubprocessError as e:
        print(f"An error occurred while playing with MPV: {e}")
        return False
...
if __name__ == "__main__":
    ...
    print("1. Play the video with MPV")
